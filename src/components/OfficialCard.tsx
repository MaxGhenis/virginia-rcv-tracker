        ...
        {evidenceCount > 0 && (
          <div className="pt-3 border-t border-[var(--slate-800)]">
            ...
            <ul className="space-y-1.5">
              {official.supportEvidence.slice(0, 3).map((evidence, i) => (
                <li key={i} className="text-xs leading-relaxed">
                  {evidence.source ? (
                    <a
                      ...
                      className="group/link flex items-start gap-1.5 text-[var(--slate-400)] hover:text-[var(--amber-400)] transition-colors"
                    >
                      <svg className="w-3 h-3 mt-0.5 flex-shrink-0 opacity-50 group-hover/link:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                      </svg>
                      <span className="line-clamp-2">{evidence.description}</span>
                    </a>
                  ) : (
                    <span className="text-[var(--slate-500)] flex items-start gap-1.5">
              ...
              {evidenceCount > 3 && (
                <li className="text-xs text-[var(--slate-600)]">
